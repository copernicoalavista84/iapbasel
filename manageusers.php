...
checklogin();
$birthyear='';$email='';$password='';$firstname='';$lastname='';$gender='';$company='';$division='';$team='';$education='';$relationship='';$actype='';
...
if(isset($_REQUEST['delete'])){	
	...
	while($test = mysql_fetch_array($tests)){
		$del=mysql_fetch_array(mysql_query("select * from tests where `tid`='".$test['tid']."'"));
		mysql_query("UPDATE `tests` SET `invites`=".($del['invites']-1)." where `tid`=$del[tid]");
		if($test['process']=='Completed')
		mysql_query("UPDATE `tests` SET `completed`=".($del['completed']-1)." where `tid`=$del[tid]");
	}
