...
<div class="contentcenbg">
		...
		if(!empty($qus)&& $cres!=$ctotqus){
			...
			if($qus['type']=='Text'){
				...
				<tr>
					...
					<th align="left" valign="top">'.$qus['question'].'</th>
				</tr>
				<tr>
				...
				for($x=1;$x<12;$x++){
					if($opt['option'.$x]!=''){
						...
						<tr>
							...
							<td align="left" valign="middle"><label><input type="radio" '.$chk.' id="weight'.$x.'" class="validate[minCheckbox[1]] checkbox" name="weight" value="'.$opt['weight'.$x].'<>'.$opt['option'.$x].'" />'.$opt['option'.$x].'</label></td>
						</tr>';
					}
				}
				echo '
    ...
    <table cellpadding="3" cellspacing="3" border="0" width="100%">
        <tr>
        <tr><td align="left" valign="middle" colspan="2"><?php echo $test['finish'];?></td></tr>
            <td align="center" colspan="2" valign="middle"><input type="submit" id="loginsub" name="finish" value="Finish"></td>
        </tr>
    </table>
    </form>
	<?php } else{?>
