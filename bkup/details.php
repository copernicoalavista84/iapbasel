...
<div class="contentcenbg">
    <table cellpadding="3" cellspacing="3" border="0" width="100%">
        ...
        <tr>
        	<td align="center" colspan="2" valign="middle">
       		<?php 
            	echo '<a href="index.php">Home</a> | <a href=profile.php>My Profile</a> | <a href=mailsettings.php>Mail Settings</a> | <a href="manageusers.php">Managing Users</a> | <a href="managetests.php">Manage Tests</a> | <a href="results.php?test='.$test['key'].'">Back</a>';
			?>
            </td>
        </tr>
        ...
        <tr>
            <td align="left" valign="middle" colspan="2" style="padding:5px 0;"><strong>Status :</strong> <?php echo $result['process']; ?></td>
        </tr>
    </table>		
	<?php	
	...
	while($cag = mysql_fetch_array($cates)){
		$type[] = $cag['category'];		
		$score=mysql_fetch_array(mysql_query("select SUM(weight) as sum, count(*) as cou from results where tid=".$cag['tid']." AND cid=".$cag['cid']." AND uid=".$tester['uid']));
 ...
 <?php 
    ...
    ?>
        ...
        <?php
       echo "<TR><th align='center' valign='middle' width='30'>#</th><th align='left' valign='middle'>Category</th><th align='center' valign='middle'>No. of Qs</th><th align='center' valign='middle'>Score</th><th align='center' valign='middle'>Max. Avg. Score</th><th align='center' valign='middle'>Avg. Score</th><th align='center' valign='middle'>His Avg. Score</th><th align='left' valign='middle'>Process</td></tr>";
        if($uc>0){
			$i = 1;
    while($cag = mysql_fetch_array($cates)){
		...
		$score=mysql_fetch_array(mysql_query("select SUM(weight) as sum, count(*) as cou from results where tid=".$cag['tid']." AND cid=".$cag['cid']." AND uid=".$tester['uid']));
		$hisscore = number_format($score['sum']/$score['cou'],2);
        echo "<tr><td align='center' valign='middle'>$i</td><td>$cag[category]</TD><TD align='center'>$rs</TD><td align='center'>$score[sum]</td><TD align='center'>$maxscore</TD><td align='center'>$avgscore</td><td align='center'>$hisscore</td><td align='left'><a href='moredetails.php?test=$test[key]&cate=$cag[key]&user=$tester[key]'>Details.</a></td></TR>";
		$i++;
    }}
