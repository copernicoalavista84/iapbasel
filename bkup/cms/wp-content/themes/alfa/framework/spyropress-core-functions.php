...
function token_repalce( $tmpl, $values ) {
    ...
    foreach ( $matches[1] as $token ) {
        $key = "{{$token}}";
        $data = str_replace( $key, $values[$token], $data );
    }
...
function get_setting_e( $item_id = '', $default = '', $is_array = false, $offset = -1 ) {
    return get_option_value( $item_id, $default, true, $is_array, $offset );
}
...
function assets_css() { global $spyropress; return $spyropress->template_url . 'assets/css/'; }
function assets_img() { global $spyropress; return $spyropress->template_url . 'assets/images/'; }
function assets_js() { global $spyropress; return $spyropress->template_url . 'assets/js/'; }
...
function spyropress_contactmethods( $contactmethods ) {
    ...
    $contactmethods['linkedin'] = __( 'Linked In', 'spyropress' );
