...
function builder_column_class( &$prev_width, $col, $extra_class = '' ) {
    ...
    // grid col size class
    $classes[] = builder_get_skeleton_class( $col->config['size'] );
    $classes[] = 'span'. $col->config['size'];

    // add span_first class
    $width = ( int )$col->config['size'];
    $new_width = $prev_width + $width;

    if ( $prev_width == 0 ) {
        $classes[] = 'alpha';
        $prev_width = $new_width;
    } elseif ( ( 16 - $new_width ) < 0 ) {
        $prev_width = $width;
        $classes[] = 'alpha';
    } else {
        $prev_width = $new_width;
 ...
 * Skeleton Classes
 */
function builder_get_skeleton_class( $size ) {
