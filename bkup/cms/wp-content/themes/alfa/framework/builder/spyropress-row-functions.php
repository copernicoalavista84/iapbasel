...
function save_row_css( $row_id, $insertion ) {
    ...
    // checks
    if ( '' == $row_id || '' == $insertion ) return false;
    
    $url = wp_nonce_url( 'admin.php?page=spyropress-theme','spyropress-theme-options' );
    
    if ( false === ( $creds = request_filesystem_credentials( $url ) ) ) return;
    ...
    if ( $wp_filesystem->is_writable( $filepath ) ) {
        ...
        $file_content = implode( '', $wp_filesystem->get_contents_array( $filepath ) );
    ...
    $wp_filesystem->put_contents( $filepath, $new_content );
}
