...
function spyropress_column_generator( $atts, $items = null ) {
    ...
    // set column cssClass
    $colclass = builder_get_skeleton_class( $columns );
    ...
    foreach ( $items as $item ) {
        ...
        if ( $columns ) {
            ...
            if ( $counter == 1 ) {
                ...
                $atts['column_class'] = $colclass. ' alpha';
                $close = true;
            }
            if ( $counter == $columns ) {
                $atts['column_class'] = $colclass. ' omega';
            }
...
function spyropress_wp_nav_menu_objects( $items, $args ) {
    ...
    $url = is_front_page() ? '#home' : home_url();
    $class = is_front_page() ? ' class="scroll"' : '';
    $items = '<li><a' . $class . ' href="' . $url . '">Home</a></li>' . $items;
