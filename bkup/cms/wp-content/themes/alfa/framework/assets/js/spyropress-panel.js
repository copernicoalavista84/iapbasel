	...
	panelUi = {
		...
		init_select_wrapper: function() {
			...
			select_changer.change(function() {
				...
				// show
				if (val) $('.' + $this.attr('id') + '.' + val).show();

			}).trigger('change');
		},
        
        init_link_generator: function() {
            $('.add-menu-link').on( 'click', function(event) {
                event.preventDefault();
                
                var $this = $(this),
                    $parent = $this.parents( '#panel-link-generator' ),
                    data = {
                        action: 'spyropress_add_menu_link',
                        menu_id: $('#menu_id', $parent).val(),
                        menu_label : $('#menu_label', $parent).val(),
                        menu_link : $('#menu_link', $parent).val()
                    };
                
                $this.next().show();
                $( '.ajax-warning', $parent ).addClass('hidden');
                ...
                $.ajax({
                    ...
                    success: function( response ) {
                        ...
                        if ( response == 'error') {
							$( '.ajax-warning', $parent ).removeClass('hidden');
							return;
						}
						
						$('#link_generator_controls', $parent).slideUp(400);
						$('.link-success', $parent).removeClass('hidden');
						return;
		...
		slider: {
			...
			gallery_uploader: function() {
				...
				$('.gallery_upload_button').live('click', function(e) {
					...
					wp.media.gallery.shortcode = function(attachments) {
						...
						// generate html
						s.gallery_generator(attachments, $gallery);

						// restore original
						wp.media.gallery.shortcode = clone;
						$string = clone(attachments);
						$shortcode.val($string.string());
						return $string;
					};

					if (typeof prev_shortcode !== 'undefined' && prev_shortcode) {
                        wp.media.gallery.edit(prev_shortcode).on('update', function(attachments) {
                            // empty the div
                            $gallery.html('');
                            $shortcode.val('');
			...
			gallery_generator: function(attachments, $gallery) {

				attachments.map(function(attachment) {
					attachment = attachment.toJSON();
					img = attachment.sizes['thumbnail'].url;

					image = $('<div data-id="' + attachment.id + '" class="gallery-item" style="display:none">' + '<img src="' + img + '" />' + '<input type="hidden" name="gallery_ids[]" value="' + attachment.id + '">' + '</div>').hide();
        ...
        repeater: {
            ...
            update_ids: function( event, ui ) {
                $groups = ui.item.parent().find( '.repeater-group' );
                newCounter = 0;
		...
		select_widget_fix: function( item ) {
            ...
            // recreate chosen
            $('.chosen', context ).each(function(index, value) {
                $elem = $(this);
                // destory chosen
                $elem.removeClass('chzn-done')
                .next('.chzn-container').remove();
			...
			setTimeout(function() {
				...
				$('.chosen-google-typo', field_id).chosen({
					...
					if (item.variants !== undefined && item.variants.length > 1) {
						...
						v.append(variants).trigger('liszt:updated');
						v.parent().show();
					} else {
						v.parent().hide();
					}
