...
function get_documentation_link( $text = 'Documentation' ) { echo '<a target="_blank" href="http://spyropress.com/codex/">' . $text . '</a>'; }
function get_support_forum_link( $text = 'Forum' ) { echo '<a target="_blank" href="http://spyropress.com/forums/">' . $text . '</a>'; }
...
function spyropress_add_shortcode_button() {

    if ( ! current_user_can( 'edit_posts' ) && ! current_user_can( 'edit_pages' ) )
        return;

    if ( 'true' == get_user_option( 'rich_editing' ) ) {
        add_filter( 'mce_external_plugins', 'spyropress_add_shortcode_tinymce_plugin' );
        add_filter( 'mce_buttons', 'spyropress_register_shortcode_button' );
    }
}

function spyropress_register_shortcode_button( $buttons ) {
    array_push( $buttons, "|", "spyroimage", "spyropress_shortcodes_button" );
    return $buttons;
}

function spyropress_add_shortcode_tinymce_plugin( $plugin_array ) {
    $plugin_array['SpyroPressShortcodes'] = framework_assets_js() . 'shortcode-editor-plugin.js';
    $plugin_array['spyroimage'] = framework_assets_js() . 'editor-image.js';
    return $plugin_array;
}

function spyropress_refresh_mce( $ver ) {
    $ver += 3;
    return $ver;
}
...
function spyropress_right_now_content() {
    ...
    foreach ( $post_types as $post_type ) {
        ...
        $text = _n( $post_type->labels->singular_name, $post_type->labels->name, intval
            ( $num_posts->publish ) );
...
function spyropress_setup_options_default( $key ) {
    ...
    if ( isset( $GLOBALS[$key] ) ) {
        ...
        //spyropress_compile_dynamic_styles( $sections, $defaults );
    }
}
...
function spyropress_save_css( $settings, $key ) {
    ...
    spyropress_compile_dynamic_styles( $settings[1], $settings[0] );
}
/* OLD */
/**
 * Compile CSS
...
function spyropress_compile_dynamic_styles( $settings, $values, $meta = false ) {
    ...
    $css_file = $spyropress->template_path . 'assets/css/dynamic.css';

    if ( !$wp_filesystem->is_readable( $less_file ) && !$wp_filesystem->is_writable( $css_file ) ) return;
    ...
    $wp_filesystem->put_contents( $css_file, $insertion );
...
add_action( 'do_feed_spyropress_export_options', 'spyropress_export_options_ajax', 1, 1 );
function spyropress_export_options_ajax() {
    global $spyropress;

    /* check for security */
    if ( ! wp_verify_nonce( $_REQUEST['security'], 'export_options' ) )
        die( 'Invalid URL..! Its look like you are tempering with url.' );

    if ( $option_name = $_GET['download'] ) {
