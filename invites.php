...
<div class="contentcenbg">
    ...
    while($fer = mysql_fetch_array($users)){
		...
		if(empty($ts))
        	echo "<tr><td align='center' valign='middle'>$i</td><td>".utf8_encode($fer[firstname])."</TD><TD>".utf8_encode($fer[lastname])."</TD><TD>$fer[company]</TD><TD>$fer[relationship]</TD><TD class='emailes'><input type=checkbox class='invite' id='invite$fer[uid]' name=row$fer[uid] value='$fer[uid]'> Send email to take test.</TD></TR>";		
		else{
			if($ts['process']=='Completed')
        		echo "<tr><td align='center' valign='middle'>$i</td><td>".utf8_encode($fer[firstname])."</TD><TD>".utf8_encode($fer[lastname])."</TD><TD>$fer[company]</TD><TD>$fer[relationship]</TD><TD class='emailes'>An email was sent & Completed.</TD></TR>";
			else
        		echo "<tr><td align='center' valign='middle'>$i</td><td>".utf8_encode($fer[firstname])."</TD><TD>".utf8_encode($fer[lastname])."</TD><TD>$fer[company]</TD><TD>$fer[relationship]</TD><TD class='emailes'>An email was sent & <input type=checkbox class='invite' id='invite$fer[uid]' name=row$fer[uid] value='$fer[uid]'>Resend email.</TD></TR>";
		}
