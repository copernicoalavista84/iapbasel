...
function checklogin(){
	if(isset($_COOKIE['email']) &&  isset($_COOKIE['password'])){
		$check=mysqli_fetch_array(mysqli_query("select * from users where email='".$_COOKIE['email']."' and status='Active'"));
		if(!empty($check)){
...
function login($val){
	...
	if(isset($email) &&  isset($password)){
		$check=mysqli_fetch_array(mysqli_query("select * from users where email='$email' and status='Active'"));
		if(!empty($check)){
...
function sendmail($campaign, $uid){
	$list=mysqli_fetch_array(mysqli_query("select * from campaigns where camid='$campaign'"));
	$options = explode(",",$list['options']);
	$user=mysqli_fetch_array(mysqli_query("select * from users where uid=$uid"));
