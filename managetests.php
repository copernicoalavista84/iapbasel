...
if (isset($_POST['addtest'])) {
    ...
    $sql = "INSERT INTO tests(uid ,testname,logo,target,instruction,finish,key)VALUES ('$uid', '" . $testname . "', '$imagename', '$target', '" . $instruction . "', '" . $finish . "', '$key')";
    echo $sql;
...
if (isset($_POST['copytest'])) {
    @extract($_POST);
    $uid = $_SESSION['login'];
    $test = mysqli_fetch_array(mysqli_query($con, "select * from tests where `tid`='$tid'"), MYSQLI_ASSOC);
    ...
    $key = str_rand(15);
    mysqli_query($con, "INSERT INTO `tests` (`uid` ,`testname` ,`logo` ,`target` ,`print` ,`instruction`, `finish` ,`max` ,`key`)VALUES ('$uid', '" . addslashes($testname) . "', '$imagename', '$test[target]', '$test[print]', '" . addslashes($test[instruction]) . "', '" . addslashes($test[finish]) . "', '$test[max]', '$key')");
    $tsid = mysqli_insert_id();
    $cates = mysqli_query($con,"select * from categories where `tid`='$tid'");
    while ($cat = mysqli_fetch_array($cates, MYSQLI_ASSOC)) {
        $key = str_rand(15);
        mysqli_query($con, "INSERT INTO `categories` (`uid` ,`tid` ,`category` ,`key`)VALUES ('$uid', '$tsid', '" . addslashes($cat[category]) . "', '$key')");
        $tcid = mysqli_insert_id();
        $questions = mysqli_query($con, "select * from questions where `tid`='$tid' and cid='" . $cat[cid] . "'");
        while ($qus = mysqli_fetch_array($questions, MYSQLI_ASSOC)) {
            $key = str_rand(15);
            $pos = mysqli_num_rows(mysqli_query($con,"select * from questions where `tid`='$tsid'")) + 1;
            if ($qus['type'] == 'Text') {
                mysqli_query($con, "INSERT INTO `questions` (`uid` ,`tid` ,`cid` ,`type` ,`pos` ,`question`,`answer`,`max`,`option`,`avg`,`key`)VALUES ('$uid', '$tsid','$tcid', '$qus[type]', '$pos','" . addslashes($qus[question]) . "', '" . addslashes($qus[answer]) . "', '$qus[max]', '$qus[option]', '$qus[avg]', '$key')");
            } else {
                mysqli_query($con, "INSERT INTO `questions` (`uid` ,`tid` ,`cid` ,`type` ,`pos` ,`question`,`answer`,`max`,`option`,`avg`,`key`)VALUES ('$uid', '$tsid','$tcid', '$qus[type]', '$pos','" . addslashes($qus[question]) . "', '" . addslashes($qus[answer]) . "', '$qus[max]', '$qus[option]', '$qus[avg]', '$key')");
                $qid = mysqli_insert_id();
                $opt = mysqli_fetch_array(mysqli_query($con, "select * from options where `qid`='$qus[qid]'"), MYSQLI_ASSOC);
                mysqli_query($con, "INSERT INTO `options` (`uid`, `tid`, `qid`, `option1`, `weight1`, `option2`, `weight2`, `option3`, `weight3`, `option4`, `weight4`, `option5`, `weight5`, `option6`, `weight6`, `option7`, `weight7`, `option8`, `weight8`, `option9`, `weight9`, `option10`, `weight10`, `option11`, `weight11`) VALUES ('$uid', '$tsid','$qid','" . addslashes($opt[option1]) . "', '$opt[weight1]', '" . addslashes($opt[option2]) . "', '$opt[weight2]', '" . addslashes($opt[option3]) . "', '$opt[weight3]', '" . addslashes($opt[option4]) . "', '$opt[weight4]', '" . addslashes($opt[option5]) . "', '$opt[weight5]', '" . addslashes($opt[option6]) . "', '$opt[weight6]', '" . addslashes($opt[option7]) . "', '$opt[weight7]', '" . addslashes($opt[option8]) . "', '$opt[weight8]', '" . addslashes($opt[option9]) . "', '$opt[weight9]', '" . addslashes($opt[option10]) . "', '$opt[weight10]', '" . addslashes($opt[option11]) . "', '$opt[weight11]')");
            }
        }
    }
    setflash("<div class='success msg'>Successfully created</div>,Your new test has been successfully created");
...
if (isset($_POST['updatetest'])) {
    $del = mysqli_fetch_array(mysqli_query($con, "select * from tests where `key`='" . $_REQUEST['edittest'] . "'"), MYSQLI_ASSOC);
    @extract($_FILES['logo']);
    if ($size && !$error) {
        ...
        $imagename = $del['logo'];
    @extract($_POST);
    mysqli_query($con, "UPDATE `tests` SET `uid`='$uid',`testname`='" . addslashes($testname) . "',`logo`='$imagename',`target`='$target',`print`='$print',`instruction`='" . addslashes($instruction) . "',`finish`='" . addslashes($finish) . "' where `tid`=$del[tid]");
    setflash("<div class='success msg'>Successfully updated</div>,Your test has been successfully updated");
    header("location: managetests.php");
    exit;
}
if (isset($_REQUEST['delete'])) {
    $del = mysqli_fetch_array(mysqli_query($con, "select * from tests where `key`='" . $_REQUEST['delete'] . "'"), MYSQLI_ASSOC);
    unlink('files/' . $del['logo']);
    @extract($del);
    mysqli_query($con, "delete from options where `tid`='" . $tid . "'");
    mysqli_query($con, "delete from questions where `tid`='" . $tid . "'");
    mysqli_query($con, "delete from results where `tid`='" . $tid . "'");
    mysqli_query($con, "delete from testassign where `tid`='" . $tid . "'");
    mysqli_query($con, "delete from categories where `tid`='" . $tid . "'");
    mysqli_query($con, "delete from tests where `tid`='" . $tid . "'");
...
if (isset($_REQUEST['addtest'])) {
    ...
    $tests = mysqli_query($con, "select * from tests");
    if (mysqli_num_rows($tests) > 0) {
        ...
        <?php
        while ($foo = mysqli_fetch_array($tests, MYSQLI_ASSOC)) {
    ...
    <?php if ($user['actype'] == 'Admin') { ?>
        ...
        <?php
        $users = mysqli_query($con, "select * from users where actype='Psychologist'");
        while ($foo = mysqli_fetch_array($users, MYSQLI_ASSOC)) {
    ...
    <?php
    $users = mysqli_query($con, "select * from users where actype='User'");
    while ($foo = mysqli_fetch_array($users, MYSQLI_ASSOC)) {
        echo "<option value='$foo[uid]'>$foo[firstname] $foo[lastname]</option>";
    }
    ?>
            ...
            <?php
            } else if (isset($_REQUEST['edittest'])) {
                $edit = mysqli_fetch_array(mysqli_query($con, "select * from tests where `key`='" . $_REQUEST['edittest'] . "'"), MYSQLI_ASSOC);
                @extract($edit);
                ?> 
        <form action="" method="post" enctype="multipart/form-data" id="myForm">
                            <?php if ($user['actype'] == 'Psychologist') { ?>
            ...
            <table cellpadding="3" cellspacing="3" border="0" align="center">
                <tr>
                    ...
                    <td rowspan="5" align="center"> <?php if ($test['logo'] == '')
                            echo '<img src="images/no_preview.jpg" alt="logo" title="logo" width="200" />';
                        else
                            echo '<img src="files/' . $test['logo'] . '" alt="logo" title="logo" width="200" />';
                        ?></td>
                </tr>
    <?php if ($user['actype'] == 'Admin') { ?>
                    <tr>
                        ...
                        <td align="left" valign="middle">
                            <select id="uid" name="uid">
        <?php
        $users = mysqli_query("select * from users where actype='Psychologist'", MYSQLI_ASSOC);
        while ($foo = mysqlii_fetch_array($users)) {
            if ($foo[uid] == $uid)
    ...
    ?>
        ...
        <?php
            ...
            if ($user['actype'] == 'Admin')
                ...
                $tests = mysqli_query($con, "select * from tests where uid='$user[uid]'");
            ...
            if ($user['actype'] == 'Admin') {
                ...
                if ($uc > 0) {
                    ...
                    while ($test = mysqli_fetch_array($tests, MYSQLI_ASSOC)) {
                        $ts = mysqli_fetch_array(mysqli_query($con, "select * from users where `uid`='$test[uid]'"));
                        $assign = mysqli_num_rows(mysqli_query($con, "select * from testassign where `tid`='$test[tid]'"));
                        if ($assign == 0)
                    ...
                    echo '<tr><td colspan="6" align="center" valign="middle">There are no tests avilable in the account</td></tr>';
            }else {
                ...
                if ($uc > 0) {
                    $i = 1;
                    while ($test = mysqli_fetch_array($tests, MYSQLI_ASSOC)) {
                        $ts = mysqli_fetch_array(mysqli_query($con, "select * from users where `uid`='$test[target]'"), MYSQLI_ASSOC);
                        if (!empty($ts))
